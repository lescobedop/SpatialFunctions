...
The next step is creating the buffers based on the locations of the clients' stores. Clients and competitors are points in the space. Every time my points have geographic coordinates and I want to perform any type of distance analysis, I change the geographic coordinates to a projected coordinates. Geographic coordinates are based on a spheroid and utilize angular units (degrees), whereas projected coordinates are based on a plane and utilize linear units. In this mini-project I use meters (sorry americans!). The transfomation is easy to do using st_transform and inluding the ESPG number of the projection (UTM 18S)
...
Now with the my point spatial entity (competitor) and polygon spatial entity (buffers), we perform a spatial join. I created a function using `st_join()`, that allow me to count how many of the 11 competitors is within the 28 buffers. 
...
The result of this function is be a data frame showing the id of the client's buffer and the number of competitors that store has at 50 meters distance. The graph is showing only the buffers with competitors within 50 meters. On a side note, there is also room for client's store consolidation.
